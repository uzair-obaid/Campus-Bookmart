...
function CartPage() {
  ...
  const handleCheckout = async () => {
    try {
      ...
      await axios
        .post("http://127.0.0.1:5000/api/orders", formData, {
          headers: {
  ...
  return (
    <main>
      <section id="cart-items">
        {cartItems.length > 0 ? (
          <>
            ...
            <div id="cart-total">
              <p>
                Bill Amount: <span>Rs {totalAmount}</span>
              </p>
            </div>
          </>
        ) : (
          <>
            ...
            <Link className=" btn-1" to="/stationery">
              Go to Stationery
            </Link>
          </>
        )}
      ...
      {cartItems.length > 0 && (
        <button id="checkout-btn" className="btn-1" onClick={handleCheckout}>
          Checkout
        </button>
      )}
