...
function StationeryPage() {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:5000/api/orders/${statusFilter}`
      );
  ...
  const markAsComplete = async (orderId) => {
    try {
      const response = await axios.put(
        `http://127.0.0.1:5000/api/orders/${orderId}/complete`
      );
  ...
  const markAsReadyToPick = async (orderId) => {
    try {
      const response = await axios.put(
        `http://127.0.0.1:5000/api/orders/${orderId}/readytopick`
      );
      if (response.status === 200) {
        fetchOrders();
      }
          ...
          {expandedOrderId === order._id && expandedOrders[order._id] && (
            <div>
              ...
              {statusFilter === "readytopick" ? (
                ...
                <button
                  className="btn-2"
                  onClick={() => markAsReadyToPick(order._id)}
                >
  ...
  return (
    <section className="product-list">
      <div className="orders">
        <div>
          <button
            className={statusFilter === "pending" ? "btn-1" : "btn-2"}
        ...
        <div>
          <button
            className={statusFilter === "readytopick" ? "btn-1" : "btn-2"}
            onClick={() => setStatusFilter("readytopick")}
          >
            Ready to Pick
          </button>
        </div>
        <div>
          <button
            className={statusFilter === "complete" ? "btn-1" : "btn-2"}
      ...
      {renderOrders(
        ordersByUser.filter((order) => order.status === statusFilter)
      )}
    </section>
  );
}
